...
export class CategoriesPage {
  ...
  startEdit(category: Category): void { this.saveError.set(null); this.announcement.set(null); this.fieldErrors.set({}); this.categoriesService.get(category.id).subscribe({ next: (detail) => { if (detail.isArchived) { this.formOpen.set(false); this.editingCategory.set(null); this.saveError.set("That category is archived and read-only."); this.loadList(); return; } this.editingCategory.set(detail); this.formOpen.set(true); this.form.reset({ name: detail.name, type: detail.type }); }, error: (error: unknown) => { this.saveError.set(this.errorMessage(error, "That category is no longer available.")); this.loadList(); } }); }
